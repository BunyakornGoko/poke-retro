...
export default function Page() {
  ...
  useEffect(() => { const poll = setInterval(() => { if (!suppressPoll.current) loadBoard() }, 1000); return () => clearInterval(poll) }, [loadBoard])
  ...
  const phase = isHost ? (board?.phaseIndex ?? 0) : (localPhase ?? board?.phaseIndex ?? 0)
  ...
  const addCard = (key: string) => { const text = (drafts[key] || '').trim(); if (!text) return; const card = { id: uid(), text, author: identity, createdAt: Date.now(), voters: [] }; const next = clone(); next.columns[key].push(card); setBoard(next); suppressPoll.current = true; fetch('/api/cards', { method: 'POST', body: JSON.stringify({ column: key, card }) }).finally(() => setTimeout(() => { suppressPoll.current = false }, 1000)); setDrafts({ ...drafts, [key]: '' }) }
  const toggleVote = (key: string, id: string) => { const next = clone(); const card = next.columns[key].find((item) => item.id === id); if (!card) return; const index = card.voters.indexOf(me); if (index >= 0) card.voters.splice(index, 1); else if (votes < 3) card.voters.push(me); setBoard(next); suppressPoll.current = true; fetch('/api/cards', { method: 'PATCH', body: JSON.stringify({ column: key, id, voters: card.voters }) }).finally(() => setTimeout(() => { suppressPoll.current = false }, 1000)) }
  const deleteCard = (key: string, id: string) => { const next = clone(); next.columns[key] = next.columns[key].filter((item) => item.id !== id); setBoard(next); suppressPoll.current = true; fetch('/api/cards', { method: 'DELETE', body: JSON.stringify({ column: key, id }) }).finally(() => setTimeout(() => { suppressPoll.current = false }, 1000)) }
  ...
  return <main className="app-shell">
    ...
    <section className="hero"><div><p className="eyebrow">SQUAD RETROSPECTIVE · SPRINT 15</p><h1>ทีมของเรา <em>เก่งขึ้น</em><br />ได้อีกแค่ไหน?</h1><p>จับมือคู่หูของคุณ แล้วมาทบทวนการเดินทางครั้งนี้ไปด้วยกัน</p></div><div className="hero-badge"><Sparkles size={17} /><span><b>{votes}/3</b> โหวตที่ใช้ไป</span></div></section>

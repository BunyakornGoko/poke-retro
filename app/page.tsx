...
export default function Page() {
  ...
  const [actionOwner, setActionOwner] = useState<Pokemon | null>(null)
  ...
  const addAction = () => { if (!actionDraft.trim()) return; const next = clone(); next.actionItems.push({ id: uid(), text: actionDraft.trim(), owner: actionOwner || identity, done: false }); save(next); setActionDraft(''); setActionOwner(null) }
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-ball" /><div><strong>Poké<span>Retro</span></strong><small>TEAM SPRINT ARENA</small></div></div><div className="header-right"><div className="trainer-chip"><PokemonAvatar pokemon={identity} size="sm" /><div><small>กำลังเล่นเป็น</small><b>{identity[0]}</b></div></div><div className="timer"><Clock3 size={17} /><b>{time}</b><input aria-label="นาที" type="number" min={1} max={60} value={minutes} onChange={(e) => setMinutes(Number(e.target.value) || 1)} /><button onClick={startTimer}>เริ่ม</button><button className="icon-button" onClick={resetTimer} aria-label="รีเซ็ตเวลา"><RotateCcw size={15} /></button></div></div></header>
